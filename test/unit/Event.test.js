...
!developmentChains.includes(network.name)
    ? describe.skip
    : describe("Event Unit Tests", async function () {
          ...
          describe("constructor", function () {
              it("initializes Event correctly", async function () {
                  ...
                  assert(
                      eventDate > purchaseStartDate &&
                          eventDate > purchaseEndDate
                  )
          ...
          describe("update Functions", function () {
              ...
              it("only owner can call update functions", async function () {
                  ...
                  const notOwnerConnectedEventContract = await event.connect(
                      notOwner
                  )
                  await expect(
                      notOwnerConnectedEventContract.updateEventDate(1)
                  ).to.be.revertedWith("Ownable: caller is not the owner")
                  await expect(
                      notOwnerConnectedEventContract.updatePurchaseStartDate(1)
                  ).to.be.revertedWith("Ownable: caller is not the owner")
                  await expect(
                      notOwnerConnectedEventContract.updatePurchaseEndDate(1)
                  ).to.be.revertedWith("Ownable: caller is not the owner")
                  await expect(
                      notOwnerConnectedEventContract.updateTicketPrice(1)
                  ).to.be.revertedWith("Ownable: caller is not the owner")
                  await expect(
                      notOwnerConnectedEventContract.updateEventName(
                          "fake name"
                      )
                  ).to.be.revertedWith("Ownable: caller is not the owner")
                  await expect(
                      notOwnerConnectedEventContract.updateEventDetails(
                          "fake details"
                      )
                  ).to.be.revertedWith("Ownable: caller is not the owner")
              })
              it("does not update eventDate when not after purchaseEndDate", async function () {
                  await expect(
                      event.updateEventDate(
                          Math.floor(inThreeDays.getTime() / 1000)
                      )
                  ).to.be.revertedWith("must be after purchaseEndDate")
              })
              it("correctly updates eventDate", async function () {
                  const newDate = Math.floor(fortnight.getTime() / 1000)
                  await event.updateEventDate(newDate)
                  const eventDate = await event.getEventDate()

                  assert.equal(eventDate.toString(), newDate.toString())
              })
              it("correctly updates purchaseStartDate", async function () {
                  const newPurchaseStartDate = Math.floor(
                      inThreeDays.getTime() / 1000
                  )
                  await event.updatePurchaseStartDate(newPurchaseStartDate)
                  const response = await event.getPurchaseStartDate()
                  assert.equal(
                      response.toString(),
                      newPurchaseStartDate.toString()
                  )
              })

              it("correctly updates purchaseEndDate", async function () {
                  const newPurchaseEndDate = Math.floor(
                      inThreeDays.getTime() / 1000
                  )
                  await event.updatePurchaseEndDate(newPurchaseEndDate)
                  const response = await event.getPurchaseEndDate()
                  assert.equal(
                      response.toString(),
                      newPurchaseEndDate.toString()
                  )
              })

              it("correctly updates TicketPrice", async function () {
                  const ticketPrice = ethers.utils.parseEther("50")
                  await event.updateTicketPrice(ticketPrice)
                  const response = await event.getTicketPrice()
                  assert.equal(response.toString(), ticketPrice.toString())
              })

              it("correctly updates EventName", async function () {
                  const eventName = "My Event"
                  await event.updateEventName(eventName)
                  const response = await event.getEventName()
                  assert.equal(response.toString(), eventName.toString())
              })

              it("correctly updates EventDetails", async function () {
                  const eventDetails = "Call to all developers"
                  await event.updateEventDetails(eventDetails)
                  const response = await event.getEventDetails()
                  assert.equal(response.toString(), eventDetails.toString())
              })
          })
